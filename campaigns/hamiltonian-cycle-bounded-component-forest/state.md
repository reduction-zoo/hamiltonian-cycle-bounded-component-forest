# Campaign state

Status: initialized. Budget: 20 research rounds, 0 used, 20 remaining, 0 mechanisms. Next: commit initialization, read local instructions, run research-session Prepare and commit it before candidate construction.

## Capability probe — 2026-09-22

- Python 3.14.7 at `/opt/homebrew/bin/python3`; uv 0.12.7 at `/Users/xiweipan/.local/bin/uv`: available. Campaign interpreter and solver pending Prepare.
- Typst 0.15.1 and pdftoppm 26.09.0 at `/opt/homebrew/bin`: available.
- Lean 4.34.0 and Lake 5.0.0-src+293d5d0 at `/Users/xiweipan/.elan/bin`: available. Mathlib not established; formalization optional and unrequested.
- External technical-writing skill at `/Users/xiweipan/.codex/plugins/cache/sci-brain/sci-brain/0.5.0/skills/how-to-technical-writing/SKILL.md`: available and read in this session; reapply for manuscript.
- Codex registered research-reviewer available, fresh-context invocation required. Exact inherited serving-model identifier and per-agent usage are not exposed. Tool metadata advertises gpt-6-astra, gpt-5.6-sol, gpt-5.6-terra, gpt-5.6-luna and gpt-5.5 overrides; none selected or tested here. Record actual isolation at review.

## Round table

| Round | Mechanism or standalone literature scope | First discriminating check | Outcome | Record |
|---|---|---|---|---|

No experience entries. Next assigned question after completion: hamiltonian-path-consecutive-ones-submatrix. No board writes or publication authorized.

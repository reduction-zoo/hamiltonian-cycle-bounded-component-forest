...
3. Write the destination `README.md`, local `AGENTS.md`, `.gitignore`, and
   ...
   the budget, the dated capability probe, an empty round table with fixed columns,
   and the next action.
...
- On the `research` preset, continue straight into
  [research-session](../research-session/SKILL.md) and start Prepare.
- Otherwise stop after the commit and print the one-line instruction for the
  ...
  campaigns/<slug>/{question,state}.md, then run Prepare.`
